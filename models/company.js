...
class Company {
  ...
  /** Find all companies.
   * 
   * Companies can be filtered using the optional searchFilter parameter
   * searchfilter can be based on the following
   * - minEmployees
   * - maxEmployees
   * - name
   * 
  ...
  static async findAll(searchFilter = {}) {
    ...
    if (minEmployees > maxEmployees) {
      throw BadRequestError("Min employees cannot exceed max employees")
    }

    if (minEmployees !== undefined) {
      queryval.push(minEmployees);
      whereexpression.push(`num_employees >= $${queryval.length}`);
    }
    ...
    if (name) {
      queryval.push(`%${name}%`);
      whereexpression.push(`name ILIKE $${queryval.length}`);
    }
